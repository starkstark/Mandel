public class MandelBrot {
	...
	public MandelBrot(MandelRequest mandelRequest) {
		...
		} else if (Context.threads > 1) {
			mandelMultiThreaded();
		}
	}

	/*
	 * private double escapeTimeDiscrete(int xi, int yi) { double x0 =
	 * mandelRequest.getX(xi); double y0 = mandelRequest.getY(yi); double x =
	 * 0.0; double y = 0.0; int iteration = 0; while (x*x + y*y < 2*2 &&
	 * iteration < mandelRequest.maxIterations) { double xtemp = x*x - y*y + x0;
	 * y = 2*x*y + y0; x = xtemp; iteration = iteration + 1; } return (double)
	 * iteration; }
	 */
	...
	private void mandelMultiThreadedBAK() {
		this.threads = new Thread[Context.threads];
	...
	private class MandelThread implements Runnable {
		...
		public void run() {
			for (int y = 0; y < mandelRequest.height; y++) {
				int tempHeight = y * mandelRequest.width;
				for (int x = this.id; x < mandelRequest.width; x += Context.threads) {
